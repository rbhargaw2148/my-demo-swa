...
const Statutory = (props) => {
    ...
    const removeAuthorization = (i) => {
        ...
        if( textFieldsArray.length === 1) {
            textFieldsArray[0].code = ""
        } else {
            textFieldsArray.splice(i,1)
        }

        setStatutories({statutes: statutoryCodesArray, textfields: textFieldsArray});
    }
        ...
        <Card>
            ...
            <table>
                ...
                {statutories.statutes.map((item, i) => {
                    return (<React.Fragment key={i}><tr id={i+1}>
                        <td>
                            ...
                            <button type="button" className="dk-blue-color link-button"><FiPlusCircle onClick={() => addStatutoryCode()}/></button>
                            <button type="button" className="dk-blue-color link-button"><FiMinusCircle onClick={()=>removeAuthorization(i)}/></button>
                            </td>
                        </tr>
                        <tr key={"othertext"+i}>
                            <td><Col md={6}>
